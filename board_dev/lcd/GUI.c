...
void Gui_DrawPixels(u16 x, u16 y, uint16_t x2, uint16_t y2, const unsigned char *p) {

    extern void SPI2_Send_framebuffer_init_to_16bit(uint8_t is16bit);

    int i;
    unsigned char picH, picL;
    LCD_SetWindows(x, y, x2, y2);//窗口设置
    //   SPI2_Send_framebuffer_init_to_16bit(1);
    //  LCD_CS_CLR;
    //  LCD_RS_SET;
    //  HAL_SPI_Transmit_DMA(&hspi2,p,(x2-x+1)*(y2-y+1));
    for (i = 0; i < (x2 - x + 1) * (y2 - y + 1); i++) {
        Lcd_WriteData_16Bit(*(uint16_t *) p);
        p += 2;
    }
    //  LCD_CS_SET;
    //  SPI2_Send_framebuffer_init_to_16bit(0);
    LCD_SetWindows(0, 0, lcddev.width - 1, lcddev.height - 1);//恢复显示窗口为全屏
}
